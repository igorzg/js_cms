...
(function() {
    ...
    var Navigation = {
        init: function () {
            var win = $(window),
                body = $(document.body);
            // open navigation
            $('.mv-navigation-call').bind('click', function openNavigation() {
                body.toggleClass('open');
            });

            win.bind('swipeRight', function (e) {
                if ($(e.target).closest('[data-carousel]').length === 0) {
                    body.addClass('open');
                }
            });
            // swipe left
            win.bind('swipeLeft', function (e) {
                if ($(e.target).closest('[data-carousel]').length === 0) {
    ...
    /**
     * Carousel
     * @type {{init: Function, setPosition: Function, left: Function, right: Function, animate: Function, resize: Function}}
     */
    var Carousel = {
        /**
         * Initialize
